...
async function main() {
  console.log("🌱 Seeding shyftforce…");
  ...
  const org = await db.organization.create({
    data: { id: ORG_UUID, slug: "platinum-security", name: "Platinum Security" },
  });
  ...
  const allMembers = [admin, ...managers, ...employees];

  // ---- Shifts (this week)
  const today = new Date(); today.setHours(0,0,0,0);
  const weekStart = new Date(today); weekStart.setDate(today.getDate() - today.getDay() + 1); // Mon
  const shiftCreates = [];
  for (let d = 0; d < 7; d++) {
    for (const e of employees) {
      // ~70% chance an employee works a given day
      if (Math.random() < 0.7) {
  ...
  // ---- Attendance: have 2 employees clocked in right now
  const now = new Date();
  for (let i = 0; i < 2; i++) {
  ...
  // Seed 16 of 66 responses (24.2% per spec)
  ...
  for (let i = 0; i < 3; i++) {
    await db.message.create({
      data: {
        fromId: managers[0].member!.id,
        toId: employees[i].member!.id,
  ...
  Org:        Platinum Security (${org.id})
  Locations:  ${locations.length}
  Members:    ${1 + managers.length + employees.length}
  Shifts:     ${shiftCreates.length}
  TS entries: ${entries}  (${flagged} flagged)
  Surveys:    1 (16/${[...employees, ...managers].length} responses)
  Login:      admin@platinum.com / password
