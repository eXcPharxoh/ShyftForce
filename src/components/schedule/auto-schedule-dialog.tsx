      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-3xl max-h-[90vh] flex flex-col">
        <header className="px-5 h-14 border-b border-ink-200 flex items-center gap-2.5 shrink-0">
          ...
          <div className="flex-1">
            ...
            <div className="text-[11px] text-ink-500 mt-0.5">Generate a full week from your rules</div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto scroll-thin">
          {step === "config" && (
            <div className="p-5 space-y-5">
              ...
              <div>
                  ...
                  <table className="w-full text-sm">
                    <thead className="bg-ink-50/60 text-[11px] uppercase text-ink-600">
                      <tr>
                        ...
                        <th className="text-center px-3 py-2">Morning<br/><span className="font-normal text-ink-500">06–14</span></th>
                        <th className="text-center px-3 py-2">Afternoon<br/><span className="font-normal text-ink-500">14–22</span></th>
                        <th className="text-center px-3 py-2">Overnight<br/><span className="font-normal text-ink-500">22–06</span></th>
                      </tr>
                    </thead>
                    <tbody>
                      {locations.map((l, i) => {
                        ...
                        return (
                          <tr key={l.id} className={i > 0 ? "border-t border-ink-100" : ""}>
                            <td className="px-3 py-2 font-medium">{l.name}</td>
                            <td className="px-3 py-2 text-center"><CovInput v={c.morning}   onChange={v => setCov(l.id, "morning", v)} /></td>
                            <td className="px-3 py-2 text-center"><CovInput v={c.afternoon} onChange={v => setCov(l.id, "afternoon", v)} /></td>
                            <td className="px-3 py-2 text-center"><CovInput v={c.overnight} onChange={v => setCov(l.id, "overnight", v)} /></td>
                          </tr>
                        );
                      })}
                    </tbody>
                ...
                <p className="text-[11px] text-ink-500 mt-1">Each number = how many people you need on that block, every day of the week.</p>
              </div>
              ...
              <div className="rounded-xl bg-brand-50/60 border border-brand-200 p-3 text-xs text-brand-900">
                <div className="font-semibold flex items-center gap-1.5"><Sparkles className="w-3.5 h-3.5" /> What the AI considers</div>
                <ul className="mt-1.5 space-y-0.5 text-brand-800">
                  ...
                  <li>• Marks slots it can't fill as <b>open shifts</b> (won't invent people)</li>
                </ul>
              </div>
            </div>
          )}
          ...
          {step === "loading" && (
            <div className="p-12 flex flex-col items-center justify-center text-center">
              <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-brand-500 to-rose-500 text-white flex items-center justify-center mb-4 animate-pulse">
                <Wand2 className="w-8 h-8" />
              </div>
              <div className="font-semibold">Building your schedule…</div>
              <div className="text-sm text-ink-500 mt-1">Reading availability, balancing hours, honoring time-off.</div>
              <div className="mt-4 flex items-center gap-2 text-xs text-ink-500"><Loader2 className="w-4 h-4 animate-spin" /> Usually takes 10-25 seconds</div>
            </div>
          )}

          {step === "error" && (
            <div className="p-8 text-center">
              <div className="w-12 h-12 rounded-2xl bg-rose-50 text-rose-600 flex items-center justify-center mx-auto mb-3"><AlertTriangle className="w-6 h-6" /></div>
        ...
        <footer className="border-t border-ink-200 p-4 flex items-center justify-between gap-3 shrink-0">
          {step === "config" && (
            <>
...
function CovInput({ v, onChange }: { v: number; onChange: (n: number) => void }) {
  return (
    <div className="inline-flex items-center gap-1">
      <button onClick={() => onChange(v - 1)} className="w-6 h-6 rounded border border-ink-200 hover:bg-ink-50 text-ink-700">−</button>
      <span className="w-8 text-center font-semibold tabular-nums">{v}</span>
      <button onClick={() => onChange(v + 1)} className="w-6 h-6 rounded border border-ink-200 hover:bg-ink-50 text-ink-700">+</button>
    </div>
  );
}
...
function ReviewView({
  ...
  return (
    <div className="p-5 space-y-4">
      <div className="card p-4 bg-gradient-to-br from-brand-50 to-rose-50 border-brand-200">
        <div className="flex items-start gap-3">
          <div className="w-9 h-9 rounded-lg bg-white text-brand-600 flex items-center justify-center"><Sparkles className="w-5 h-5" /></div>
          <div className="flex-1 min-w-0">
            <div className="font-semibold text-sm">AI summary</div>
            <p className="text-sm text-ink-700 mt-0.5">{proposal.summary}</p>
            {proposal.warnings.length > 0 && (
              <div className="mt-2 space-y-1">
                {proposal.warnings.map((w, i) => (
                  <div key={i} className="text-xs text-amber-900 bg-amber-100/80 rounded-md px-2 py-1 flex items-start gap-1.5">
      ...
      <div className="space-y-2">
        {days.map((d, i) => {
          ...
          return (
            <div key={key} className="border border-ink-200 rounded-xl overflow-hidden">
              <div className="px-3 py-2 bg-ink-50 flex items-center justify-between">
                <div className="font-semibold text-sm">{DAY_LABELS[i]} · {dateLabel(d)}</div>
                <div className="text-[11px] text-ink-500">{items.length} shifts</div>
              </div>
              <ul className="divide-y divide-ink-100">
                {items.map(({ shift, idx }) => {
                  ...
                  return (
                    <li key={idx} className={`px-3 py-2 flex items-center gap-3 ${isExcl ? "bg-ink-50/60 opacity-50" : ""}`}>
                      <input type="checkbox" checked={!isExcl} onChange={() => toggle(idx)} className="rounded border-ink-300 text-brand-500 focus:ring-brand-500" />
                      <div className="flex-1 min-w-0">
                        <div className="text-sm font-medium truncate">
                          {isOpen ? <span className="text-amber-700">Open shift</span> : shift.memberName}
                          <span className="text-ink-500"> · {shift.position}</span>
                        </div>
                        <div className="text-[11px] text-ink-500 truncate">{shift.locationName} · {shift.startTime}–{shift.endTime}{shift.rationale ? <span className="text-ink-400"> · {shift.rationale}</span> : null}</div>
                      </div>
                      <span className={isOpen ? "badge bg-amber-50 text-amber-700" : "badge bg-brand-50 text-brand-700"}>
                        {isOpen ? "open" : `${hoursBetween(shift.startTime, shift.endTime).toFixed(0)}h`}
                      </span>
                    </li>
                  );
                })}
                {items.length === 0 && <li className="px-3 py-3 text-xs text-ink-500 italic">No shifts this day.</li>}
              </ul>
            </div>
          );
        })}
...
function Stat({ label, value, total, tone = "ink" }: { label: string; value: number | string; total?: number; tone?: "ink" | "emerald" | "amber" }) {
  const map: any = { ink: "text-ink-900", emerald: "text-emerald-700", amber: "text-amber-700" };
  return (
    <div className="rounded-xl border border-ink-200 p-2.5">
      <div className={`text-xl font-bold ${map[tone]}`}>{value}{total != null && <span className="text-sm text-ink-400 font-normal"> / {total}</span>}</div>
