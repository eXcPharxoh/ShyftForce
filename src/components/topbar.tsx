...
export function Topbar({ name, role, image }: { name: string; role?: string; image?: string | null }) {
  ...
  return (
    <>
      <header className="bg-white/80 backdrop-blur-xl border-b border-ink-200/70 h-16 sticky top-0 z-30 flex items-center px-6 gap-3">
        {/* Co-pilot trigger */}
        <button
          ...
          className="group flex-1 max-w-2xl flex items-center gap-2.5 px-3.5 h-10 rounded-xl
                     ...
                     hover:border-brand-300 hover:bg-brand-50/40 hover:shadow
        ...
        >
          ...
          <span className="text-sm text-ink-500 flex-1 truncate group-hover:text-ink-700">
        ...
        <div className="relative" ref={menuRef}>
          <button
            onClick={() => setOpen(v => !v)}
            className="flex items-center gap-2 p-1 pl-1.5 pr-2 rounded-xl hover:bg-ink-100 transition"
          >
            {image
            ...
            <div className="text-left hidden md:block">
              <div className="text-[12px] font-semibold leading-none text-ink-900">{name}</div>
          ...
          {open && (
            <div className="absolute right-0 top-12 w-60 card p-1.5 z-50 animate-scale-in origin-top-right">
              <div className="px-2.5 py-2.5 border-b border-ink-100 mb-1 flex items-center gap-2">
                {image
              ...
              <div className="my-1 mx-1 border-t border-ink-100" />
              <button
                onClick={() => signOut({ callbackUrl: "/login" })}
                className="w-full flex items-center gap-2 px-2.5 py-2 rounded-lg text-sm hover:bg-rose-50 text-rose-600 transition"
              >
...
function MenuLink({ href, icon: Icon, label }: { href: string; icon: any; label: string }) {
  return (
    <Link href={href} className="flex items-center gap-2 px-2.5 py-2 rounded-lg text-sm text-ink-700 hover:bg-ink-50 transition">
