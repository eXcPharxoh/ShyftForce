...
export function DashboardHero({
  ...
  return (
    <div className="relative overflow-hidden rounded-3xl border border-ink-200/70 bg-white p-6 md:p-8 shadow-card animate-fade-up">
      {/* Decorative bg */}
      ...
      <div className="relative flex flex-col md:flex-row md:items-end md:justify-between gap-4">
        <div>
          ...
          <h1 className="text-[34px] leading-[1.05] font-bold tracking-tight-2 text-ink-900">
            {g}, <span className="text-gradient-brand">{firstName}</span>
          </h1>
          <p className="text-sm text-ink-600 mt-2">
            Here's what's happening at <span className="font-semibold text-ink-900">{orgName}</span> today.
          </p>
          <div className="mt-4 flex flex-wrap items-center gap-2 text-xs">
            <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-ink-100/70 text-ink-700">
        ...
        <Link
          href="/schedule"
          className="group inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-ink-900 text-white text-sm font-semibold
                     shadow-soft hover:bg-ink-800 transition self-start"
        >
